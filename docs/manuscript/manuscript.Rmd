...
As described by previous publications [@wallinga_how_2007], we can define a function $n(a)$ that defines the expected number of new infections created today (time $t$) by a single person who was infected $a$ days ago (at time $t-a$). Therefore, to calculate $N_a$ (the total expected number of new infections created today by all individuals who became infected $a$ days ago), we multiply $n(a)$ by the number of new infections created $a$ days ago ($I_{t-a}$).
...
\end{equation}
```
Then to determine the total expected number of new infections today ($I_t$), we sum $N_a$ for all values of $a$ (i.e. sum up the expected number of new infections created by each infected individuals based on the age of their infection). If we treat $a$ and $t$ instead of discrete days, we integrate over all values of $a$ to produce Equation \@ref(eq:renewal-eq).
...
\end{equation}
```
Next we can substitute the generation interval function ($g(a)$) into Equation \@ref(eq:renewal-eq) by recognizing that the generation interval distribution is equal to $n(a)$ normalized by the total number of infections a single infected person is expected cause (i.e. $R_t$).
...
```{=tex}
\begin{equation} 
  I_t = \int_a^\infty{I_{t-a}\frac{g(a)}{R_t}}da
  (\#eq:sub-in-rt-1)
\end{equation}
```
```{=tex}
...
\end{equation}
```
If we make the assumption that there is minimal variance in the generation interval distribution (i.e. $g(a)$ equals approximately 1 when $a$ equals the mean generation interval ($d$) and $g(a)$ equals approximately zero for all other values of $a$), then Equation \@ref(eq:renewal-eq) can be simplified to:
...
# 
rt_estimates_error_summary <- rt_estimates %>%
  filter(time > 14) %>%
  group_by(simulation_run) %>%
  summarise(dm_test = list(dm.test(rt.simple_mean_error, rt.cori_mean_error)))
...
panel_D <- rt_estimates %>% 
  ...
  ggplot(aes(x = time, y = estimation_error)) + geom_line(aes(group = simulation_run, color = Method), alpha = 0.2) + 
  facet_wrap(~Method, ncol = 1) +
  custom_theme() + theme(strip.background = element_blank(), strip.text = element_blank()) + 
  scale_color_manual(values=cbPalette) +
  scale_fill_manual(values=cbPalette)

full_figure_top = ggarrange(panel_A, panel_B, panel_C, ncol=1, nrow = 3, align = "hv", labels=c("A", "B", "C"), common.legend = T)
full_figure_bottom = ggarrange(panel_D, labels=c("D"), common.legend = T)
ggarrange(full_figure_top, full_figure_bottom, ncol = 2, common.legend = T)
