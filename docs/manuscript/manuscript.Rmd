...
\end{equation}
```
The generation interval distribution is equal to $n(a)$ normalized by the total number of infections a single infected person is expected cause (i.e. $R_t$).
...
\end{equation}
```
Thus, Equation \@ref(eq:generation-interval-eq) can be substituted into Equation \@ref(eq:renewal-eq) to produce:

```{=tex}
\begin{equation} 
  \frac{I_t} = \int_a^\infty{I_{t-a}\frac{g(a)}{R_t}}da
  (\#eq:sub-in-rt-1)
\end{equation}
```
```{=tex}
