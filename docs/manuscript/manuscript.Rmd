...
A subset of previous methods for estimating $R_t$ are based on the following relationship:
...
\end{equation}
```
$I_t$ is the number of new cases produced on day $t$; $g(a)$ is the infectiousness of a person infected $a$ days ago - $g(a)$ can be thought of as equivalent to to the generation interval distribution. To understand the derivation of this equation, we refer readers to [@wallinga_how_2007].
...
```{r estimation-inaccParams,  fig.cap = "\\textbf{$R_t$ estimation error when the mean or standard deviation of the generation interval is mis-specified}. Similar to \\textbf{Figure 1}, this figure demonstrate the distribution of absolute error when estimating $R_t$ for 100 simulated epidemics.  \\textbf{(A)} Distribution of errors when the mean of the generation interval is mis-specified as 2 days too long. The average \\textbf{(B)} Distribution of errors when the standard deviation is mis-specified as 3 days too long."}
