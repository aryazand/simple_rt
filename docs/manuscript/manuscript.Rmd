...
From a public health perspective, the $R_t$ can be a method to communicate whether an epidemic is growing in a region. However, this communication may be hindered by the fact that many of the current methods for estimating $R_t$ require advanced mathematics to understand and to apply [@cori_new_2013; @wallinga_different_2004; @wallinga_how_2007]. Here, we describe how $R_t$ can be approximated by a ratio of new infections on day $t$ and day $t-d$, where $d$ is the mean generation interval of the infection (Equation 1). The average *generation interval* is defined as the average time from when a primary case becomes infectious to when a secondary case becomes infectious.
...
Then to determine the total expected number of new infections today ($I_t$), we sum $I_{ta}$ for all values of $a$. In other words, $I_t$ can be calculated by taking all individuals infected prior to $t$, calculating the expected number of new infections each one will produce based on the age of their infection, and summing all those values together. If we treat $a$ as continuous variables instead of discrete days, we can integrate over all values of $a$, as shown in \cref{eq:renewal-eq-1}.

```{=tex}
\begin{equation} 
  I_t = \int_a^\infty{I_{ta}}da
  (\#eq:renewal-eq-1)
\end{equation}
```
```{=tex}
...
```{=tex}
\begin{equation} 
  I_t = \int_a^\infty{I_{t-a}g(a)R_t}da
  (\#eq:sub-in-rt-1)
\end{equation}
```
```{=tex}
